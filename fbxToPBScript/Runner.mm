...
- (void)setupVerticesAndNodes:(NSString *) filename {
	...
	// Find file
	NSString *filepath = inputFilePath;
...
- (void) writeFile: (NSData *)dataToWrite {
	...
	NSString *filePath=[NSString stringWithFormat:outputFilePath];
...
-(void) run {
	[self setupVerticesAndNodes:@"Dabang_final"];
}
